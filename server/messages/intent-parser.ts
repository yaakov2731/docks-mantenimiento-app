...
export type IntentType =
  // Tareas / Reclamos
  | 'task_accept'         // "1", "recibida", "acepto", "ok tomo", "sí puedo"
  | 'task_reject'         // "2", "no puedo", "no llego", "ocupado", "franco"
  | 'task_start'          // "empiezo", "iniciando", "voy para allá"
  | 'task_pause'          // "pauso", "espero material", "bloqueado"
  | 'task_resume'         // "continúo", "retomo", "sigo"
  | 'task_complete'       // "listo", "terminé", "completado", "finalicé"
  | 'task_progress'       // Cualquier nota de avance libre
  | 'task_help'           // "no tengo herramientas", "necesito ayuda"
  // Rondas
  | 'round_start'         // "1", "inicio ronda"
  | 'round_pause'         // "2", "pauso ronda"
  | 'round_finish'        // "3", "terminé ronda", "ronda lista"
  | 'round_observation'   // "4", "con observación", "hay problema"
  | 'round_unable'        // "5", "no pude", "no llegué"
  // Asistencia
  | 'attendance_entrada'  // "entrada", "llegué", "entro"
  | 'attendance_salida'   // "salida", "me voy", "salgo"
  | 'attendance_almuerzo_inicio' // "almuerzo", "voy a comer"
  | 'attendance_almuerzo_fin'    // "volví del almuerzo", "de vuelta"
  // Admin
  | 'admin_pendientes'    // "1", "ver pendientes", "qué hay"
  | 'admin_asignar'       // "2", "asignar"
  | 'admin_buscar'        // "3", "buscar #123"
  | 'admin_help'          // "4", "ayuda"
  // General
  | 'greeting'            // "hola", "buenas"
  | 'status_query'        // "qué tengo", "mis tareas", "qué me toca"
...
export interface ParsedIntent {
  ...
  /** Confianza 0-1 */
  confidence: number
  /** Número de opción si el mensaje fue solo un dígito */
  option?: string
  /** Texto libre extraído (nota, observación, etc.) */
  freeText?: string
  /** ID de tarea/reclamo/ronda extraído del mensaje */
  entityId?: number
  /** Indica si el mensaje es solo un número (ej: "1", "2") */
  isNumericOption: boolean
}
...
interface Rule {
  ...
  /** Confianza base para esta regla */
  confidence: number
}
 ...
 */
export function parseIntent(rawMessage: string): ParsedIntent {
  ...
  // Try each rule
  let bestMatch: { rule: Rule; confidence: number } | null = null
  ...
  if (!bestMatch) {
    ...
    if (isNumericOption) {
      if (option === '1') return { type: 'task_accept', confidence: 0.7, option, isNumericOption: true, entityId }
      if (option === '2') return { type: 'task_reject', confidence: 0.7, option, isNumericOption: true, entityId }
      if (option === '3') return { type: 'round_finish', confidence: 0.65, option, isNumericOption: true, entityId }
      if (option === '4') return { type: 'round_observation', confidence: 0.65, option, isNumericOption: true, entityId }
      if (option === '5') return { type: 'round_unable', confidence: 0.65, option, isNumericOption: true, entityId }
    }
 ...
 */
export function buildUnknownIntentResponse(context: 'employee' | 'admin'): string {
  if (context === 'admin') {
    return [
 ...
 */
export function buildIntentConfirmation(intent: IntentType, params?: {
  tareaId?: number
  titulo?: string
  tiempo?: string
}): string | null {
