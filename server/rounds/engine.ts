...
export type BuildOccurrencesInput = {
  plantillaId: string
...
export type RoundOccurrenceRow = BuildOccurrencesInput & {
  horaProgramada: string
  programadoAtIso: string
  estado: 'pendiente'
}

export type ResolveOccurrenceStateInput = {
  programadoAtIso: string
  recordatorioEnviadoAtIso: string | null
  confirmadoAtIso: string | null
  nowIso: string
}

export type RoundOccurrenceState = 'pendiente' | 'cumplido' | 'vencido'
...
export function pickScheduleForDate(
  fechaOperativa: string,
  schedules: readonly RoundSchedule[]
): RoundSchedule | undefined {
  return (
    schedules.find((schedule) => schedule.modoProgramacion === 'fecha_especial' && schedule.fechaEspecial === fechaOperativa) ??
    schedules.find((schedule) => schedule.modoProgramacion === 'semanal' && schedule.diaSemana === getDayOfWeek(fechaOperativa))
  )
}

export function buildOccurrencesForDate(input: BuildOccurrencesInput): RoundOccurrenceRow[] {
  ...
  const occurrences: RoundOccurrenceRow[] = []
...
export function resolveOccurrenceState(input: ResolveOccurrenceStateInput): RoundOccurrenceState {
...
function getDayOfWeek(fechaOperativa: string): number {
