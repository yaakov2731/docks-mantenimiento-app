...
import { CalendarDays, Clock3, Coffee, LogIn, LogOut, Store, Timer, UserRoundCheck, Users, Wallet } from 'lucide-react'
...
function getEmployeeStatus(events: any[], empId: number) {
  const last = events
    .filter((e: any) => e.empleadoId === empId)
    .sort((left: any, right: any) => new Date(right.timestamp).getTime() - new Date(left.timestamp).getTime())[0]
  const onShift = last?.tipo === 'entrada' || last?.tipo === 'fin_almuerzo'
  const onLunch = last?.tipo === 'inicio_almuerzo'
  return {
    last,
    onShift,
    onLunch,
    label: onLunch ? 'Almuerzo' : onShift ? 'En turno' : 'Fuera de turno',
    tone: onLunch
...
export default function GastronomiaAsistencia() {
  ...
  const stats = useMemo(() => {
    const onShift = employees.filter((emp: any) => getEmployeeStatus(events, emp.id).onShift).length
    const onLunch = employees.filter((emp: any) => getEmployeeStatus(events, emp.id).onLunch).length
  ...
  }, [employees, events, todayKey, liquidacion])
  ...
  return (
    <DashboardLayout title="Reloj Gastronomía">
      <div className="space-y-5">
        <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-emerald-700">
                  <Store size={14} />
                  {sector === 'todos' ? 'Todos los locales' : sectorLabel(sector)}
                </div>
                <h1 className="mt-3 font-heading text-[28px] md:text-[36px] font-semibold leading-tight text-slate-950">
                  Reloj y tarjetas de Gastronomía
                </h1>
                <p className="mt-2 max-w-2xl text-sm text-slate-500">
                  Vista separada de mantenimiento: empleados gastronómicos, estado del turno, actividad y días trabajados por local.
                </p>
              </div>
              <div className="rounded-[26px] bg-slate-950 px-5 py-4 text-right text-white">
                <div className="text-[10px] uppercase tracking-[0.24em] text-white/45">Reloj central</div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <MetricCard label="En turno" value={stats.onShift} hint="Última marca abierta." icon={UserRoundCheck} tone="border-emerald-200 bg-emerald-50 text-emerald-900" />
            ...
            <MetricCard label="Estimado" value={formatCurrency(stats.total)} hint="Mes seleccionado." icon={Wallet} tone="border-sky-200 bg-sky-50 text-sky-900" />
          </div>
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-[1fr_0.8fr]">
          <div className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="font-heading text-xl font-semibold text-slate-900">Tarjetas del equipo</h2>
                <p className="text-sm text-slate-500">Solo personal gastronómico.</p>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {employees.map((emp: any) => {
                const status = getEmployeeStatus(events, emp.id)
                ...
                return (
                  <div key={emp.id} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div className="mt-4 rounded-[18px] bg-slate-950 p-4 text-white">
                      <div className="text-[10px] uppercase tracking-[0.22em] text-white/40">Última marca</div>
                      <div className="mt-1 flex items-center justify-between gap-3">
                        <div className="font-semibold text-emerald-300">{actionLabel(status.last?.tipo ?? 'Sin marca')}</div>
                        <div className="font-mono text-lg font-semibold">{formatTime(status.last?.timestamp)}</div>
                      </div>
                      <div className="mt-2 text-xs text-white/45">{formatDateTime(status.last?.timestamp)}</div>
                    </div>
