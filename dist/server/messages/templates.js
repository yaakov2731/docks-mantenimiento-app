...
exports.nextTaskAutoAssigned = nextTaskAutoAssigned;
exports.operationalTaskAssigned = operationalTaskAssigned;
exports.roundReminder = roundReminder;
exports.roundOverdue = roundOverdue;
exports.roundObservationAlert = roundObservationAlert;
exports.roundAssignment = roundAssignment;
exports.leadAssigned = leadAssigned;
exports.newComplaintAlert = newComplaintAlert;
exports.taskRejectedAlert = taskRejectedAlert;
exports.overdueTasksSummary = overdueTasksSummary;
exports.botDisconnectedAlert = botDisconnectedAlert;
exports.attendanceEntradaConfirmed = attendanceEntradaConfirmed;
exports.attendanceSalidaConfirmed = attendanceSalidaConfirmed;
// ─── Helpers ────────────────────────────────────────────────────────────────
 ...
 */
function nextTaskAutoAssigned(params) {
    const emoji = prioridadEmoji(params.prioridad);
    return [
 ...
 */
function operationalTaskAssigned(params) {
    const emoji = prioridadEmoji(params.prioridad);
    const lines = [
    ...
    lines.push('', 'Respondé con una opción:', '✅ 1. Tarea recibida', '❌ 2. No puedo tomarla');
    return lines.join('\n');
}
 ...
 */
function roundObservationAlert(params) {
    return [
 ...
 */
function roundAssignment(params) {
    return [
...
function leadAssigned(params) {
    ...
    if (params.telefono)
        lines.push(`📞 Teléfono: ${params.telefono}`);
    if (params.email)
        lines.push(`📧 Email: ${params.email}`);
    if (params.rubro)
        lines.push(`🏪 Rubro: ${params.rubro}`);
    if (params.tipoLocal)
        lines.push(`🏢 Tipo de local: ${params.tipoLocal}`);
    if (params.fuente)
        lines.push(`📡 Fuente: ${params.fuente}`);
    if (params.mensaje) {
        lines.push('', `💬 *Mensaje:*`, params.mensaje);
    }
    lines.push('', '👉 Contactalo a la brevedad.', 'Registrá el seguimiento en el panel.');
    return lines.join('\n');
}
 ...
 */
function newComplaintAlert(params) {
    const emoji = prioridadEmoji(params.prioridad);
    return [
 ...
 */
function taskRejectedAlert(params) {
 ...
 */
function overdueTasksSummary(params) {
    if (params.tareas.length === 0)
        return '';
    const lines = [
 ...
 */
function botDisconnectedAlert(params) {
    return [
 ...
 */
function attendanceEntradaConfirmed(params) {
    return [
 ...
 */
function attendanceSalidaConfirmed(params) {
    return [
